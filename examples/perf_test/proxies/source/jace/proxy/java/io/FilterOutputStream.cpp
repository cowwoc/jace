
#ifndef JACE_PROXY_JAVA_IO_FILTEROUTPUTSTREAM_H
#include "jace/proxy/java/io/FilterOutputStream.h"
#endif

/**
 * Standard Jace headers needed to implement this class.
 *
 */
#ifndef JACE_JARGUMENTS_H
#include "jace/JArguments.h"
#endif
using jace::JArguments;

#ifndef JACE_JMETHOD_H
#include "jace/JMethod.h"
#endif
using jace::JMethod;

#ifndef JACE_JFIELD_H
#include "jace/JField.h"
#endif
using jace::JField;

#ifndef JACE_JCLASS_IMPL_H
#include "jace/JClassImpl.h"
#endif
using jace::JClassImpl;


/**
 * Headers for the classes that this class uses.
 *
 */
#ifndef JACE_TYPES_JVOID_H
#include "jace/proxy/types/JVoid.h"
#endif
#ifndef JACE_TYPES_JINT_H
#include "jace/proxy/types/JInt.h"
#endif
#ifndef JACE_PROXY_JAVA_IO_IOEXCEPTION_H
#include "jace/proxy/java/io/IOException.h"
#endif

BEGIN_NAMESPACE_4( jace, proxy, java, io )

/**
 * The Jace C++ proxy class source for java/io/FilterOutputStream.
 * Please do not edit this source, as any changes you make will be overwritten.
 * For more information, please refer to the Jace Developer's Guide.
 *
 */
#ifndef VIRTUAL_INHERITANCE_IS_BROKEN
  #define FilterOutputStream_INITIALIZER : ::jace::proxy::java::io::OutputStream( NO_OP ), ::jace::proxy::java::lang::Object( NO_OP )
#else
  #define FilterOutputStream_INITIALIZER : ::jace::proxy::java::io::OutputStream( NO_OP )
#endif

FilterOutputStream::FilterOutputStream( ::jace::proxy::java::io::OutputStream p0 ) FilterOutputStream_INITIALIZER {
  ::jace::JArguments arguments;
  arguments << p0;
  jobject localRef = newObject( arguments );
  setJavaJniObject( localRef );
  JNIEnv* env = ::jace::helper::attach();
  ::jace::helper::deleteLocalRef( env, localRef );
}

void FilterOutputStream::write( ::jace::proxy::types::JInt p0 ) {
  ::jace::JArguments arguments;
  arguments << p0;
  ::jace::JMethod< ::jace::proxy::types::JVoid >( "write" ).invoke( *this, arguments );
}

void FilterOutputStream::write( ::jace::JArray< ::jace::proxy::types::JByte > p0 ) {
  ::jace::JArguments arguments;
  arguments << p0;
  ::jace::JMethod< ::jace::proxy::types::JVoid >( "write" ).invoke( *this, arguments );
}

void FilterOutputStream::write( ::jace::JArray< ::jace::proxy::types::JByte > p0, ::jace::proxy::types::JInt p1, ::jace::proxy::types::JInt p2 ) {
  ::jace::JArguments arguments;
  arguments << p0 << p1 << p2;
  ::jace::JMethod< ::jace::proxy::types::JVoid >( "write" ).invoke( *this, arguments );
}

void FilterOutputStream::flush() {
  ::jace::JArguments arguments;
  ::jace::JMethod< ::jace::proxy::types::JVoid >( "flush" ).invoke( *this, arguments );
}

void FilterOutputStream::close() {
  ::jace::JArguments arguments;
  ::jace::JMethod< ::jace::proxy::types::JVoid >( "close" ).invoke( *this, arguments );
}

/**
 * The following methods are required to integrate this class
 * with the Jace framework.
 *
 */
FilterOutputStream::FilterOutputStream( jvalue value ) FilterOutputStream_INITIALIZER {
  setJavaJniValue( value );
}

FilterOutputStream::FilterOutputStream( jobject object ) FilterOutputStream_INITIALIZER {
  setJavaJniObject( object );
}

FilterOutputStream::FilterOutputStream( const FilterOutputStream& object ) FilterOutputStream_INITIALIZER {
  setJavaJniObject( object.getJavaJniObject() );
}

FilterOutputStream::FilterOutputStream( const NoOp& noOp ) FilterOutputStream_INITIALIZER {
}

const JClass* FilterOutputStream::staticGetJavaJniClass() throw ( JNIException ) {
  static JClassImpl javaClass( "java/io/FilterOutputStream" );
  return &javaClass;
}

const JClass* FilterOutputStream::getJavaJniClass() const throw ( JNIException ) {
  return FilterOutputStream::staticGetJavaJniClass();
}

JEnlister< FilterOutputStream> FilterOutputStream::enlister;

END_NAMESPACE_4( jace, proxy, java, io )

#ifndef PUT_TSDS_IN_HEADER
  template <> jace::ElementProxy< jace::proxy::java::io::FilterOutputStream>::ElementProxy( jarray array, jvalue element, int index ) : 
    jace::proxy::java::io::FilterOutputStream( element ), Object( NO_OP ), mIndex( index ) {
    JNIEnv* env = ::jace::helper::attach();
    parent = static_cast<jarray>( ::jace::helper::newGlobalRef( env, array ) );
  }
  template <> jace::ElementProxy< jace::proxy::java::io::FilterOutputStream>::ElementProxy( const jace::ElementProxy< jace::proxy::java::io::FilterOutputStream>& proxy ) : 
    jace::proxy::java::io::FilterOutputStream( proxy.getJavaJniObject() ), Object( NO_OP ), mIndex( proxy.mIndex ) {
    JNIEnv* env = ::jace::helper::attach();
    parent = static_cast<jarray>( ::jace::helper::newGlobalRef( env, proxy.parent ) );
  }
#endif
#ifndef PUT_TSDS_IN_HEADER
  template <> ::jace::JFieldProxy< ::jace::proxy::java::io::FilterOutputStream>::JFieldProxy( jfieldID fieldID_, jvalue value, jobject parent_ ) : 
    ::jace::proxy::java::io::FilterOutputStream( value ), Object( NO_OP ), fieldID( fieldID_ ) {
    JNIEnv* env = ::jace::helper::attach();

    if ( parent_ ) {
      parent = ::jace::helper::newGlobalRef( env, parent_ );
    }
    else {
      parent = parent_;
    }

    parentClass = 0;
  }
  template <> ::jace::JFieldProxy< ::jace::proxy::java::io::FilterOutputStream>::JFieldProxy( jfieldID fieldID_, jvalue value, jclass parentClass_ ) : 
    ::jace::proxy::java::io::FilterOutputStream( value ), Object( NO_OP ), fieldID( fieldID_ ) {
    JNIEnv* env = ::jace::helper::attach();

    parent = 0;
    parentClass = static_cast<jclass>( ::jace::helper::newGlobalRef( env, parentClass_ ) );
  }
  template <> ::jace::JFieldProxy< ::jace::proxy::java::io::FilterOutputStream>::JFieldProxy( const ::jace::JFieldProxy< ::jace::proxy::java::io::FilterOutputStream>& object ) : 
    ::jace::proxy::java::io::FilterOutputStream( object.getJavaJniValue() ), Object( NO_OP ) {

    fieldID = object.fieldID; 

    if ( object.parent ) {
      JNIEnv* env = ::jace::helper::attach();
      parent = ::jace::helper::newGlobalRef( env, object.parent );
    }
    else {
      parent = 0;
    }

    if ( object.parentClass ) {
      JNIEnv* env = ::jace::helper::attach();
      parentClass = static_cast<jclass>( ::jace::helper::newGlobalRef( env, object.parentClass ) );
    }
    else {
      parentClass = 0;
    }
  }
#endif

