 ...
 *
 */
template <class ElementType> class JArray: public ::org::jace::proxy::JObject
{
public:
	/**
	...
	{
		jobject localRef = ::org::jace::JArrayHelper::newArray(size, ElementType::staticGetJavaJniClass());
	...
	template <class T> JArray(const std::vector<T>& values): JObject(0)
	{
		...
		jobjectArray localArray = ::org::jace::JArrayHelper::newArray(values.size(), ElementType::staticGetJavaJniClass());
	 ...
	 *
	 */
	::org::jace::proxy::types::JInt length() const
	{
		#ifdef JACE_CHECK_NULLS
			if (!static_cast<jobject>(*this))
				throw ::org::jace::JNIException("[JArray::length] Can not retrieve the length of a null array.");
		#endif

		if (_length == -1)
			_length = ::org::jace::JArrayHelper::getLength(static_cast<jobject>(*this));
		return _length;
	}
	...
	ElementProxy<ElementType> operator[](const int& index)
	{
		#ifdef JACE_CHECK_NULLS
			if (!static_cast<jobject>(*this))
				throw ::org::jace::JNIException("[JArray::operator[]] Can not dereference a null array.");
		#endif

		#ifdef JACE_CHECK_ARRAYS
			if (index >= length())
				throw ::org::jace::JNIException("[JArray::operator[]] invalid array index.");
		#endif

		jvalue localElementRef = ::org::jace::JArrayHelper::getElement(static_cast<jobject>(*this), index);
	 ...
	 */
	virtual const ::org::jace::JClass& getJavaJniClass() const throw (::org::jace::JNIException)
	{
	 ...
	 * @throw JNIException if an error occurs while trying to retrieve the class.
	 */
	static const ::org::jace::JClass& staticGetJavaJniClass() throw (JNIException)
	{
	...
	public:
			...
			end(_end)
		{
				...
				if (_begin < 0 || _begin > parent->length())
					throw ::org::jace::JNIException("[JArray::Iterator::Iterator] begin is out of bounds.");
				if ((_end < _begin && _end != -1) || end > parent->length())
					throw ::org::jace::JNIException("[JArray::Iterator::Iterator] end is out of bounds.");
			#endif
		...
		Iterator operator++()
		{
				...
				if (current >= parent->length())
					throw ::org::jace::JNIException("[JArray::Iterator::operator++] can not advance iterator out of bounds.");
			#endif
		...
		Iterator operator--()
		{
			#ifdef JACE_CHECK_ARRAYS
				if (current == 0)
					throw ::org::jace::JNIException("[JArray::Iterator::operator--] can not rewind iterator out of bounds.");
			#endif
		...
		ElementProxy<ElementType> operator*()
		{
				...
				if (current < 0 || current >= parent->length())
					throw ::org::jace::JNIException("[JArray::Iterator::operator*] can not dereference an out of bounds iterator.");
			#endif
 ...
 */
  ...
  #include "org/jace/JArray.tsd"
#else
  #include "org/jace/JArray.tsp"
#endif

#endif // #ifndef ORG_JACE_JARRAY_H
