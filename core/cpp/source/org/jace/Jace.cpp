...
#include "org/jace/Peer.h"
using org::jace::Peer;

#include "org/jace/VmLoader.h"
using ::org::jace::VmLoader;

#include "org/jace/VirtualMachineShutdownError.h"
using org::jace::VirtualMachineShutdownError;

#include "org/jace/VirtualMachineRunningError.h"
using org::jace::VirtualMachineRunningError;
...
void catchAndThrow(JNIEnv* env)
{
  ...
  // cout << "org::jace::catchAndThrow() - Discovered an exception: " << endl;
  // print(jexception);
  ...
  //  cout << "org::jace::catchAndThrow() - Retrieving the exception class type..." << endl;
  ...
  if (env->ExceptionOccurred())
    ...
    string msg = string("org::jace::catchAndThrow()\n") +
  ...
  while (true)
    ...
    if (it == getFactoryMap()->end())
      ...
      if (env->ExceptionOccurred())
			{
        env->ExceptionDescribe();
        throw JNIException("org::jace::catchAndThrow()\nAn error occurred while trying to call "
    ...
    // cout << "org::jace::catchAndThrow() - Throwing the exception " << endl;
    // print(jexception);
...
JNIEnv* attachImpl(JavaVM* jvm, const jobject threadGroup, const char* name, const bool daemon) throw (JNIException)
{
  ...
  if (result != 0)
	{
    string msg = string("org::jace::attach\n") +
...
void enlist(JFactory* factory)
{
  ...
  //  cout << "org::jace::enlist - Enlisted " << name << endl;
}
...
jobject newLocalRef(JNIEnv* env, jobject ref) throw (JNIException)
{
  ...
  if (!localRef)
	{
    throw JNIException(string("org::jace::newLocalRef\n") +
...
jobject newGlobalRef(JNIEnv* env, jobject ref) throw (VirtualMachineShutdownError, JNIException)
{
  ...
  if (!globalRef)
	{
    throw JNIException(string("org::jace::newGlobalRef\n") +
...
::org::jace::Peer* getPeer(jobject jPeer)
{
  JNIEnv* env = attach();
  ...
  return reinterpret_cast< ::org::jace::Peer* >(nativeHandle);
}
